...
# ============================================================
# Rate Limiting（资源滥用防护）
# ============================================================

# 全局调用记录（按用户维度可扩展为 Redis）
_CALL_LOCK = threading.Lock()
_CALL_HISTORY: dict = {}  # {user_id: [timestamp, ...]}

# 默认限制：每小时最多 20 次生成
DEFAULT_RATE_LIMIT = 20
DEFAULT_RATE_WINDOW = 3600  # 1 小时（秒）
...
def rate_limit(
    ...
    :param window: 时间窗口（秒）
    :param user_key: 用户标识（None 时使用全局限制）
    """
    def decorator(func: Callable):
        @wraps(func)
        def wrapper(*args, **kwargs):
            now = time.time()
            key = user_key or "_global_"

            with _CALL_LOCK:
                history = _CALL_HISTORY.get(key, [])
                # 清理过期记录
                history = [t for t in history if now - t < window]
                _CALL_HISTORY[key] = history

                if len(history) >= max_calls:
                    raise RuntimeError(
                        f"调用频率超限（每 {window//3600} 小时最多 {max_calls} 次），"
                        f"请 {int((history[0] + window - now) // 60)} 分钟后再试"
                    )
...
def check_nsfw_text(text: str, custom_keywords: Optional[List[str]] = None) -> None:
    """
    检查文本是否包含 NSFW 内容

    :param text: 待检查文本
    :param custom_keywords: 自定义关键词列表（追加到默认列表）
    :raises ValueError: 检测到违规内容时抛出
    """
    keywords = list(_NSFW_KEYWORDS)
    if custom_keywords:
    ...
    for keyword in keywords:
        if keyword.lower() in text_lower:
            raise ValueError(f"输入文本包含不当内容（匹配关键词：{keyword}），已拒绝生成")
...
class VideoProcessor:
    ...
    def add_watermark(
        ...
        # 位置映射（x:y）
        pos_map = {
            "SE": "(W-w-20):(H-h-20)",   # 右下角
            "SW": "20:(H-h-20)",            # 左下角
            "NE": "(W-w-20):20",            # 右上角
            "NW": "20:20",                  # 左上角
        }
        pos = pos_map.get(position, pos_map["SE"])
        ...
        cmd = [
            ...
            f"fontcolor=white@0.4:fontsize=24:x={pos.split(':')[0]}:y={pos.split(':')[1]}",
            "-c:a", "copy",
            "-c:v", "libx264", "-preset", "ultrafast",
            wm_path,
        ]
        self._run_safe(cmd, "添加水印")
        return wm_path
